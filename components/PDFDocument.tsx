...
import type { Devis, Facture, Client } from '@/types'
...
const styles = StyleSheet.create({
  ...
  logo: { fontSize: 26, fontFamily: 'Helvetica-Bold', color: '#0B2440', letterSpacing: -1 },
  ...
  tableHeader: { flexDirection: 'row', backgroundColor: '#0B2440', color: 'white', padding: '8px 12px', fontFamily: 'Helvetica-Bold', fontSize: 9, textTransform: 'uppercase', letterSpacing: 0.8 },
  ...
  col1: { flex: 2.5 }, col2: { width: 50, textAlign: 'center' }, col3: { width: 55, textAlign: 'right' },
  col4: { width: 50, textAlign: 'right' }, col5: { width: 70, textAlign: 'right', fontFamily: 'Helvetica-Bold' },
  ...
  conformite: { fontSize: 7.5, color: '#AAAAAA', marginTop: 4 },
})
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View>
            ...
            {isDevis && (doc as Devis).date_validite && (
              <Text style={[styles.docDate, { color: '#DD5A2A' }]}>Valable jusqu&apos;au {new Date((doc as Devis).date_validite!).toLocaleDateString('fr-FR')}</Text>
            )}
        ...
        <View style={styles.section}>
          <View style={{ flex: 1, marginRight: 32 }}>
            ...
            {artisanInfo.tva && <Text style={styles.address}>TVA : {artisanInfo.tva}</Text>}
        ...
        <View style={styles.footer}>
          {isDevis && (doc as Devis).date_validite && (
            <Text>Ce devis est valable jusqu&apos;au {new Date((doc as Devis).date_validite!).toLocaleDateString('fr-FR')}. Pour l&apos;accepter, veuillez signer et retourner avec la mention &quot;Bon pour accord&quot;.</Text>
          )}
          {!isDevis && (
            <Text>Paiement à réception de facture. En cas de retard, des pénalités de retard au taux légal seront appliquées, ainsi qu&apos;une indemnité de 40 € (art. L441-10 C. com.).</Text>
          )}
          ...
          <Text style={styles.conformite}>
            Document généré par Worklin · Conforme réforme facturation électronique 2027 (PPF/PDP) · Données conservées 10 ans
          </Text>
        </View>
      </Page>
    </Document>
