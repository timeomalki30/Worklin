...
import {
  ...
  Sparkles, BarChart3, Globe, Settings, LogOut, Wrench,
} from 'lucide-react'
...
export default function ArtisanLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (a) {
        setMetier(a.metier)
        const { count } = await supabase.from('demandes').select('*', { count: 'exact', head: true })
          .eq('artisan_id', a.id).eq('statut', 'nouveau')
  ...
  const isActive = (href: string) =>
    href === '/dashboard/artisan' ? pathname === href : pathname.startsWith(href)
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: '#F5F1E8' }}>

      {/* ── Sidebar ── */}
      <aside className="w-[240px] bg-navy-800 flex flex-col flex-shrink-0 overflow-y-auto">

        {/* Logo */}
        <div className="px-5 py-5 border-b border-white/[0.08]">
          <Link href="/" className="flex items-center gap-3 no-underline">
            <div className="w-9 h-9 bg-terra-500 rounded-xl grid place-items-center relative overflow-hidden flex-shrink-0 shadow-glow">
            ...
            <div>
              ...
              <div className="text-terra-400 text-[10px] font-bold uppercase tracking-widest mt-0.5">Dashboard</div>
            </div>
          </Link>
        </div>
          ...
          {NAV.map(({ href, label, icon: Icon, badge }) => {
            ...
            return (
              ...
              >
                ...
                {badge === 'IA' && (
                  <span className="text-[9px] font-black bg-terra-500 text-white px-1.5 py-0.5 rounded-md tracking-wide">IA</span>
                )}
                {href === '/dashboard/artisan/vitrine' && demandesCount > 0 && (
                  <span className="text-[10px] font-bold bg-terra-500 text-white w-5 h-5 rounded-full grid place-items-center flex-shrink-0">
                    {demandesCount}
                  </span>
                )}
              </Link>
        ...
        <div className="px-3 pb-4 pt-2 border-t border-white/[0.08]">
          <div
            className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/10 cursor-pointer group transition-all"
            onClick={handleLogout}
          >
            <div
              className="w-8 h-8 rounded-xl bg-terra-500 grid place-items-center text-white font-bold text-sm flex-shrink-0"
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-white/40 text-xs truncate capitalize mt-0.5">{metier}</div>
            </div>
            <LogOut size={14} className="text-white/30 group-hover:text-white/70 flex-shrink-0 transition-colors" />
          </div>
        </div>
      </aside>
